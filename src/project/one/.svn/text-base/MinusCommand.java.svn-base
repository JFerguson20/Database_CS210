...
public class MinusCommand implements ICommand {
	...
	private Pattern pattern = Pattern.compile(
			"^\\s*minus\\s+(.+\\s+)and\\s+(.+\\s*);", Pattern.CASE_INSENSITIVE); // If
																					// two
																					// table
																					// names

	private Pattern p2 = Pattern.compile(
			"^(\\s*minus\\s*(\\(.*\\)+)\\s*and\\s*)", Pattern.CASE_INSENSITIVE); // first
																					// part
																					// of
																					// double
																					// query
																					// lists
	private Pattern p3 = Pattern.compile("(^\\s*minus\\s+(\\w+)\\s+and\\s*)",
			Pattern.CASE_INSENSITIVE); // for if first part is tableName and
										// second part is queryList
	...
	public void execute() throws MyException {
		...
		Database.getDB().minusTable(queryList.trim(), queryList2.trim());
