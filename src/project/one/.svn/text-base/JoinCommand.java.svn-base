...
public class JoinCommand implements ICommand {
	...
	private Pattern pattern = Pattern.compile(
			"^\\s*join\\s+(.+\\s+)and\\s+(.+\\s*);", Pattern.CASE_INSENSITIVE); // If
																				// two
																				// table
																				// names

	private Pattern p2 = Pattern.compile(
			"^(\\s*join\\s*(\\(.*\\)+)\\s*and\\s*)", Pattern.CASE_INSENSITIVE); // first
																				// part
																				// of
																				// double
																				// query
																				// lists
	private Pattern p3 = Pattern.compile("(^\\s*join\\s+(\\w+)\\s+and\\s*)",
	...
	public void execute() throws MyException {
		...
		Database.getDB().joinTable(queryList.trim(), queryList2.trim());
